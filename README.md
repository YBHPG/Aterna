...
### 10. Бизнес-логика жизненного цикла письма

- **Инкапсуляция адресата:** Письма теперь пишутся только "в будущее себе" (адресат автоматически извлекается из JWT-токена, поле `recipientEmail` исключено).
- **Редактирование (`PATCH /messages/:id`):** Возможность редактировать текст письма строго в первые 24 часа после его создания. Канал доставки (Telegram, Email или оба) можно изменять всё время, пока письмо находится в статусе `pending`.
- **Блокировка чтения:** Если письмо находится в ожидании отправки (`pending`) и прошло более 24 часов с момента создания, его контент "запечатывается" и скрывается сервером (`isLocked: true`).

### 11. Управление профилем пользователя (Profile API)

- Создан независимый `ProfileModule` для управления личными данными авторизованного юзера.
- Добавлены DTO и безопасные эндпоинты для обновления электронной почты (`UpdateEmailDto`), пароля (`UpdatePasswordDto`) и имени профиля (`UpdateNameDto`).
- **Безопасность Email:** Смена почты происходит через механизм `pendingEmail` (старая почта работает до подтверждения новой). Вход в аккаунт с неподтвержденной почтой строго заблокирован до подтверждения.
- **Сброс пароля:** Реализован флоу "Забыл пароль" с использованием 6-значных одноразовых паролей (OTP), которые могут доставляться как на email, так и в Telegram.
- Проведен рефакторинг архитектуры: DTO пользователей перенесены в выделенную директорию `src/users/dto/`.
- Реализована привязка Telegram-аккаунта с использованием механизма Deep Linking. Добавлен эндпоинт генерации одноразовой ссылки (`GET /profile/telegram-link`) и обработчик webhook-запросов от бота (`POST /telegram/webhook`).

### 12. Разработка клиентской части (Frontend SPA) - MVP
...
### 13. Интерактивные черновики и отложенная регистрация (Frontend)

- **Главная страница:** Форма написания письма перенесена на маршрут `/` и теперь доступна публично.
- **Черновики:** Введенный текст и выбранная дата автоматически сохраняются в `localStorage`. Черновик восстанавливается при возврате на страницу.
- **Отложенная регистрация:** При отправке письма неавторизованным пользователем происходит редирект на `/register?intent=send_draft`. После входа система автоматически отправляет сохраненный черновик.

### 14. Расширенный интерфейс писем и профиль (Frontend)

- **Профиль:** Добавлена страница `/profile` с формами для обновления имени, email и пароля (через OTP), а также смарт-кнопками интеграций.
- **Жизненный цикл письма в UI:** Реализовано отображение трех состояний письма: редактирование (в течение 24 часов), блокировка (после 24 часов до отправки) и чтение (отправленные или отмененные).

### 15. Продвинутый UI/UX и Тёмная тема

- **Дизайн-система:** Полный переход на CSS-переменные для управления цветами. Добавлена полноценная нативная тёмная тема (`prefers-color-scheme: dark`) с инверсией иконок.
- **Уведомления и Отмена действий:** Интеграция `react-hot-toast` вместо системных алертов. Реализован паттерн "Отложенного выполнения" (Undo action) при удалении писем со стильной анимацией таймера.
- **Анимации:** Добавлены скелетон-загрузки (Skeleton Loading), плавные транзиции изменения размеров текстовых полей, CSS-анимации появления и исчезновения элементов списков.
- **Mobile Experience:** Кастомный скрипт auto-resize для `textarea`, предотвращающий "скачки" экрана и потерю фокуса при вводе длинного текста на мобильных устройствах (Android/iOS).
- **Email шаблоны:** Все транзакционные письма (доставка, подтверждение регистрации, OTP-коды) свёрстаны в новом фирменном стиле (шрифты Cormorant/Inter, скругленные карточки).
...
- **Consumer (`EmailDeliveryProcessor`):**
    ...
    - Распределяет уведомления: отправляет письмо на email (только при подтвержденном адресе) и в Telegram (с автоматической обрезкой длинных текстов до 4000 символов и добавлением inline-кнопки на дашборд).
...
| MessagesModule (Жизненный цикл писем)  | Ready  |
| ProfileModule (Управление профилем)    | Ready  |
| AuthModule (Восстановление пароля)     | Ready  |
...
| Frontend: Интерактивные черновики      | Ready  |
| Frontend: Профиль и просмотр писем     | Ready  |
| Frontend: UI/UX, Тёмная тема, Анимации | Ready  |
| Email Templates (Дизайн писем)         | Ready  |
