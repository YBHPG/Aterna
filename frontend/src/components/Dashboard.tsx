...
export const Dashboard: React.FC = () => {
    ...
    const handleCancel = async (id: string) => {
        ...
        setMessages((prevMessages) =>
            prevMessages.map((msg) => (msg._id === id ? { ...msg, status: "cancelled" } : msg)),
        );
            ...
            <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center space-x-4">
                    <button
                        ...
                        className="px-4 py-2 text-sm text-gray-700 transition-colors bg-gray-200 border border-gray-300 rounded hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-400"
                    >
                        Профиль
                    </button>
                    <button
                        onClick={fetchMessages}
                        className="px-4 py-2 text-sm text-white transition-colors bg-blue-600 rounded hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    >
                        Обновить
                    </button>
                </div>
