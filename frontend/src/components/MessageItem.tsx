...
export const MessageItem: React.FC<MessageItemProps> = ({ message, onCancel }) => {
  ...
  const renderStatusBadge = (status: Message['status']) => {
    switch (status) {
      case 'pending':
        return <span className="px-2 py-1 text-xs font-semibold text-yellow-800 bg-yellow-200 rounded-full">В очереди</span>;
      case 'sent':
        return <span className="px-2 py-1 text-xs font-semibold text-green-800 bg-green-200 rounded-full">Отправлено</span>;
      case 'error':
        return <span className="px-2 py-1 text-xs font-semibold text-red-800 bg-red-200 rounded-full">Ошибка</span>;
      case 'cancelled':
        return <span className="px-2 py-1 text-xs font-semibold text-gray-800 bg-gray-200 rounded-full">Отменено</span>;
      default:
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-white border rounded-md shadow-sm gap-4">
      <div>
        <p className="font-medium text-gray-800">{message.recipientEmail}</p>
        <p className="text-sm text-gray-500">{new Date(message.triggerDate).toLocaleString()}</p>
      </div>
